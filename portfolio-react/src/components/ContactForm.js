...
const ContactForm = ({ onAdd }) => {
  ...
  const handleOnChange = (position) => {
    const updatedGameCheck = gameChoices.map((choice) =>
      choice.id === position
        ? { ...choice, isPlayed: !choice.isPlayed }
        : choice
    );

    setGameChoices(updatedGameCheck);

    const filteredGameChoices = updatedGameCheck.filter(
      (choice) => choice.isPlayed === true
    );
  ...
  return (
    <div className="" style={{ marginTop: "60px", marginBottom: "120px" }}>
      ...
      <form
        className="container px-5 py-3 bg-dark text-white"
        onSubmit={onSubmit}
      >
        ...
        <div className="input-group col">
          <span className="input-group-text">
            What kind of games do you play? (You can choose none of them)
          </span>

          {gameChoices.map((gameChoice, index) => {
            return (
              <label key={index} className="form-check-label m-1">
                <input
                  type="checkbox"
